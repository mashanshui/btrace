...
public final class RheaTrace3 {
    ...
    /** 清空已有数据并开启一次全进程抓栈性能统计；结束调用必须在同一线程完成。 */
    public static void beginStackTiming() {
        TraceGlobal.beginStackTiming();
    }

    /**
     * 结束当前线程开启的抓栈性能统计，返回本次会话的汇总日志。
     * 汇总包含完整抓栈耗时分布、总耗时和限频浪费耗时；会话无效时返回空字符串。
     */
    public static String endStackTiming() {
        return TraceGlobal.endStackTiming();
    }
}
